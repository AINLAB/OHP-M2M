 ...
 * deactivate the IPU plug-in.
 * In the start() method, the Activator registers the IPU Controller service on the registry 
 * to make it available for the CORE plug-in. Then, it starts tracking the CORE SCL service to
 * use it once it the CORE plug-in is activated.
	...
	public void start(BundleContext bundleContext) throws Exception
	{
		...
		sclServiceTracker = new ServiceTracker<Object, Object>(context,
			...
			public Object addingService(ServiceReference<Object> reference) 
			{
				...
				final CardioVascularMonitor bloodPressureMonitor = new CardioVascularMonitor(
						sclService);
				new Thread() {
					public void run()
					{
