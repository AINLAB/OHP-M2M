				...
				for (Element ele : root.elements()) {
					...
					if(isInteger(str)){
						...
						if(value > 90 && value < 100){
							System.out.println("Spo2 ---> " + value);
							Spo2Device.spo2 = value;
						}
					}
					getElements(ele);
				}
	...
	public void listenToSpo2() {
		new Thread() {
			...
			public void ObserveBloodPresure(String uri) {
				...
				CoapClient client = new CoapClient(uri + "/SpO2Monitor");

				CoapObserveRelation relation1 = client.observe(mySpo2Handler);
			}
			...
			public void run() {
				...
				ObserveBloodPresure("coap://192.168.1.19:5683");
