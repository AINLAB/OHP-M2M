...
import com.brian.knu.coap.strenghtfitnessequipment.device.StrengthFitnessDevice;
...
import com.brian.knu.coap.strenghtfitnessequipment.dim.elements.DevConfigurationID;
import com.brian.knu.coap.strenghtfitnessequipment.dim.elements.Simple;
import com.brian.knu.coap.strenghtfitnessequipment.dim.elements.SystemID;
import com.brian.knu.coap.strenghtfitnessequipment.dim.elements.SystemModel;
import com.brian.knu.coap.strenghtfitnessequipment.dim.elements.SystemTypeSpecList;


/**
 * The StrengthFitnessMonitor class creates one strength Fitness application called "COAP_STRENGTHFITNESS".
 ...
 * 
 * */
public class StrengthFitnessMonitor{
					...
					public static String sensorId = "COAP_STRENGTHFITNESS";
					public static int sensorValue = 0;
		
					public StrengthFitnessMonitor(SclService sclService)
					{
						core = sclService;
					}
					...
					public void createCVResources()
					{
						...
						// Create the COAP_STRENGTHFITNESS application
						...
						if (response.getStatusCode().equals(StatusCode.STATUS_CREATED))
						{
							...
							content = StrengthFitnessDevice.getDescriptiorRep(sclId, sensorId,
									tempId);
							...
							content = StrengthFitnessDevice.getDIMRep(sensorId, null);
							targetId = sclId + "/applications/" + sensorId
			...
			private CoapHandler myStrenghtFitnessHandler = new CoapHandler() 
			{
				@Override
				...
				{
					String deviceName = "StrengthFitness";
			...
			public void listenToCV() {
				new Thread() {
					...
					public void observerStrengthFitness(String uri) {
		
						CoapClient client = new CoapClient(uri + "/strenghtfitnessmonitor"); // This creates a coap client to 
																							//listen to strengthfitness 
		
						CoapObserveRelation relation1 = client
								.observe(myStrenghtFitnessHandler);
					}
					...
					public void run() {
						...
						observerStrengthFitness("coap://192.168.1.12:5683");
